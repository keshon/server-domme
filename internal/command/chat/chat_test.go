...
func TestEverySubcommandIsOfferedToDiscord(t *testing.T) {
	...
	for _, name := range []string{subChannel, subBrief, subStatus, subForget, subRole, subAbout, subWhy, subBackends, subReflect, subWake, subDelete} {
		if !offered[name] {
			t.Errorf("%q is handled in Run but never offered in SlashDefinition, so nobody can run it", name)
		}
	}
	if len(offered) != 11 {
		t.Errorf("SlashDefinition offers %d subcommands, want 11 — an unhandled one fails closed", len(offered))
	}
}
