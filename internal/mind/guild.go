...
// PushMessage adds a message to short-term buffer and bumps activity.
...
// ApplyActivityDecay reduces ActivityScore by exponential decay. Call from scheduler.
func (g *GuildState) ApplyActivityDecay(decayPerSec float64, now time.Time) {
	g.mu.Lock()
	defer g.mu.Unlock()
	if g.activity == nil {
		return
	}
	elapsed := now.Sub(g.activity.LastMsgAt).Seconds()
	if elapsed < 0 {
		elapsed = 0
	}
	// decay: score *= e^(-decayPerSec * elapsed), simplified as linear decay per tick
	g.activity.Score *= 1.0 - decayPerSec
