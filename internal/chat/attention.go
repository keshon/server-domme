...
func (s *Service) maybeReach(guildID string, channels []string, p storage.MindPerson, now time.Time) {
	level, ok := mind.ParseAttention(p.Attention)
	if !ok || level == mind.AttentionOff {
		return
	}
	...
	r := mind.Reach{
		Now:        now,
		Level:      level,
		Longing:    mind.FeelLonging(now, p.LastExchangeAt, active, warmth),
		Warmth:     warmth,
		Irritation: mind.IrritationNow(p.Irritation, p.IrritatedAt, now),
	...
	item := mind.Deferred{
		...
		Journal: s.journalOpen(storage.MindJournal{
			...
			Attitude: mind.Attitude(warmth, r.Irritation, 0),
...
// noticeExchange records that someone spoke to her, and honours a request to
// back off from someone who had opted in.
func (s *Service) noticeExchange(m *discordgo.MessageCreate, person *storage.MindPerson, content string, now time.Time) {
	...
	if person == nil || person.Attention == "" || !mind.WantsPeace(content) {
		return
	}
	if err := s.store.SetMindAttention(m.GuildID, m.Author.ID, "", now); err != nil {
