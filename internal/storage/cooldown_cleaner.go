...
// cooldownSweepInterval is how often expired task cooldowns are swept.
// Cooldowns are compared against wall-clock time on read, so this only controls
// how long dead entries linger in the store, never whether a cooldown is
// enforced.
const cooldownSweepInterval = time.Minute

// RunCooldownCleaner clears expired task cooldowns until ctx is done. It
// blocks, so run it in a goroutine.
