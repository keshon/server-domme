...
// from the other side. The guarantee is only worth as much as this test: without
// it, adding a middleware or renaming SkipAuditLog would quietly start recording
// who confessed, and nothing user-visible would change until someone read the log.
...
// The control: without the opt-out the logger must still wrap, or the test above
// would pass just as happily on a middleware that logs nothing at all.
